...
use crate::font::{unicode_glyph_index, variation_sequence_base_codepoints};
...
pub fn rewrite_font(src_path: &str, dst_path: &str, keep_runes: &[u32]) -> Result<()> {
    ...
    let mut effective_keep: BTreeSet<u32> = keep_runes.iter().copied().collect();
    effective_keep.extend(variation_sequence_base_codepoints(&font_data));
    ...
    if !effective_keep.is_empty() && char_to_gid.is_empty() {
